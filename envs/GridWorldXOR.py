...
class GridWorldEnvXOR(gym.Env):
    ...
    COLOURS = {"0": [0.8, 0.8, 0.8, 1], "#": [0.0, 0.0, 0.0, 0.0], "S": [0.4, 1, 0.4, 1], "E": [0.4, 1, 0.4, 1], "1": [0.4, 0.4, 1, 1], "2":[1, 0.4, 0.4, 1], "3":[0.4, 1, 0.4, 1]}
    ...
    def __init__(self, MAP=MAP, length=5, random_length=False, active=False, fix_start=False, seed=None, max_episode_step=100, fully_obs=False, goal_obs=False, continual=False, render_mode = 'human'):   
        ...
        # row0 = row0[:3]+" #"*length+row0[3:]
        # row1 = row1[:3]+" #"*length+row1[3:]
        # row2 = row2[:3]+" 0"*length+row2[3:]
        ...
        # Transition probs
        self.states = []
        for goal in [(-1,-1),self.goal1,self.goal2]:
            if goal==(-1,-1) and self.goal_obs: continue
    ...
    def get_pos_obs(self, goal):
        state = self.goals[goal[0]]
        if self.fully_obs: return state
        position = state
        state = int(self.grid[position[0]][position[1]])
        state = self.obs_onehot[state].copy()
    ...
    def stepP(self, state, action, fully_obs=True):
        ...
        for goal in goals:
            ...
            reward, done = 0, False
            if   goal[0]==goal[1] and position == self.goal1: reward, done =  1, True
            elif goal[0]!=goal[1] and position == self.goal2: reward, done =  1, True
            elif goal[0]==goal[1] and position == self.goal2: reward, done =  -1, True
            elif goal[0]!=goal[1] and position == self.goal1: reward, done =  -1, True
            
            if self.continual and (position in [self.goal1, self.goal2]): 
                done, goal_observed = False, self.goal_obs
                position = self.start_position
                idx1 = self.start_goal if self.start_goal!=None else np.random.randint(len(self.goals))
                idx2 = self.start_goal if self.start_goal!=None else np.random.randint(len(self.goals))
                goal = (idx1,idx2)
    ...
    def step(self, action):
        ...
        if   self.goal[0]==self.goal[1] and self.position == self.goal1: reward, self.done =  1, True
        elif self.goal[0]!=self.goal[1] and self.position == self.goal2: reward, self.done =  1, True
        elif self.goal[0]==self.goal[1] and self.position == self.goal2: reward, self.done =  -1, True
        elif self.goal[0]!=self.goal[1] and self.position == self.goal1: reward, self.done =  -1, True
