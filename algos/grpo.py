...
from algos.sb3_buffers import RolloutBuffer
...
class GRPO(OnPolicyAlgorithm):
    """
    Group Relative Policy Optimization algorithm (GRPO) (clip version)
    ...
    def __init__(
        ...
        if rollout_buffer_kwargs == None:
            rollout_buffer_kwargs = {"grpo":True}
        else:
            rollout_buffer_kwargs.update({"grpo":True})
        super().__init__(
            policy,
            env,
