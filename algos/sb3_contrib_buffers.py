...
def pad(
    ...
    # ]
    seq = [th.tensor(tensor[indices], device=device) for indices in seq_indices]
    tensor = th.nn.utils.rnn.pad_sequence(seq, batch_first=True, padding_value=padding_value)
    return tensor
...
def pad_and_flatten(
    ...
    """
    return pad(seq_start_indices, seq_end_indices, seq_indices, device, tensor, padding_value).flatten()
...
def create_sequencers(
    ...
    seq_end_indices = np.min( [seq_end_indices, seq_start_indices+n_stack], axis=0)
    seq_indices = [ np.arange(start, end+(end<len(seq_start)))
    ...
    local_pad = partial(pad, seq_start_indices, seq_end_indices, seq_indices, device)
    local_pad_and_flatten = partial(pad_and_flatten, seq_start_indices, seq_end_indices, seq_indices, device)
    return seq_start_indices, local_pad, local_pad_and_flatten

def create_sequencers_adaptive(
    env,
    actions: int,
    n_stack: int,
    episode_starts: np.ndarray,
    ...
    for i, action in enumerate(actions):
        ...
        if len(action)==1:
            env_action, memory_action = env.env_method("split_env_action_memory_action", action[0], indices=0)[0]
        else:
            env_action, memory_action = env.env_method("split_env_action_memory_action", action, indices=0)[0]
        # memory_action = 0 # Uncommenting this line reduces Adaptive Stacking to Frame Stacking (Slinding Window)
        if seq_start[i+1]: # Next episode
            seq_indices.append(indices)
            indices = [i+1]
        else:
            memory_action = (n_stack - 1) - memory_action
            if memory_action<len(indices): indices.pop(memory_action)
            indices.append(i+1)
    seq_indices.append(indices)
...
class RecurrentRolloutBuffer(RolloutBuffer):
    ...
    def _get_samples(
        ...
        if self.adaptive:
            self.seq_start_indices, self.pad, self.pad_and_flatten = create_sequencers_adaptive(
                self.env, self.actions[batch_inds],
                self.n_stack, self.episode_starts[batch_inds], env_change[batch_inds], self.device
            )
        else:
